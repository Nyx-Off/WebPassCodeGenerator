...
class UIController {
    ...
    updateStrengthIndicator(result) {
        ...
        if (!this.elements.strengthBar || !this.elements.strengthText || !this.elements.crackTime) {
            return; // Skip if elements are not present
        }
    ...
    generateQRCode(password) {
        ...
        this.elements.qrContainer.appendChild(qrDiv);
    ...
    addToHistory(password) {
        const strengthText = this.elements.strengthText ? 
            this.elements.strengthText.textContent.replace('STRENGTH: ', '') : 
            'UNKNOWN';
